...
const BASE_URL = CLIENT_URL.endsWith("/")
...
export const sendInviteEmail = async (to, sessionData) => {
  ...
  const html = `
    <div style="font-family: sans-serif; max-width: 600px; margin: auto; border: 1px solid #eee; padding: 20px; border-radius: 10px;">
      ...
      <a href="${joinLink}" style="display: inline-block; background: #DCEF62; color: #1A1A1A; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">
        View Invitation
      </a>
      <p style="margin-top: 20px; font-size: 0.8rem; color: #999;">
        If you don't have a SkillSprint account, please ignore this email.
      </p>
    </div>
  `;
...
export const sendPairProgrammingInvite = async (
  to,
  { inviterName, projectName, shareUrl }
) => {
  const link = shareUrl.startsWith("http")
    ? shareUrl
    : `${BASE_URL}${shareUrl.startsWith("/") ? "" : "/"}${shareUrl}`;
  ...
  const html = `
    <div style="font-family: sans-serif; max-width: 600px; margin: auto; border: 1px solid #eee; padding: 20px; border-radius: 10px;">
      ...
      <hr style="border: 0; border-top: 1px solid #eee; margin: 20px 0;">
      <a href="${link}" style="display: inline-block; background: #DCEF62; color: #1A1A1A; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">
        Open Project
      </a>
    </div>
  `;
...
export const sendBoardInvite = async (
  to,
  { inviterName, boardName, shareUrl }
) => {
  const link = shareUrl.startsWith("http")
    ? shareUrl
    : `${BASE_URL}${shareUrl.startsWith("/") ? "" : "/"}${shareUrl}`;
...
export const sendOTPEmail = async (to, otp, type = "signup") => {
  const subject =
    type === "signup"
      ? "SkillSprint Signup OTP"
      : "SkillSprint Password Reset OTP";
  ...
  const html = `
    <div style="font-family: sans-serif; max-width: 600px; margin: auto; border: 1px solid #eee; padding: 20px; border-radius: 10px;">
      ...
      <p>
        ...
        <strong style="font-size: 1.2rem; color: #DCEF62; background: #333; padding: 4px 8px; border-radius: 4px;">
          ${otp}
        </strong>
      </p>
